...
`)
	...
	ghost := ghostStyle.Render("                             👻")
	subtitle := subtitleStyle.Render("\n        Trust Layer TUI for GhostSpeak")
	version := subtitleStyle.Render(fmt.Sprintf("\n              v%s | Built with Charm\n", Version))

	return banner + ghost + subtitle + version
}
...
func runVersion(cmd *cobra.Command, args []string) {
	...
	fmt.Println(titleStyle.Render("Boo 👻"))
	fmt.Println()
